	...
	public class Fisherman extends Building
	{
		...
		override public function update()
		{
			...
			for(var ind:int = 0; ind < buildingContent.length ; ind++)
			{
				...
				if(fishingChair.level < 6) // fishingChair not ready
				{
					fishingChair.level++;
				}
				else if(fishingChair.beingWorked) // fishingChair being worked
				{
					// do nothing
				}
				else if(fishingChair.worker) // fishingChair has a worker (but not being woreked)
				{
					if(fishingChair.worker.col == rally_col && fishingChair.worker.row == rally_row && fishingChair.worker.isIdle()) // the minion is in the fisherman location
					{
						fishingChair.startWork();
					}
					else if(!fishingChair.worker.targetPosition) // the minion isn't on the way (and not in the fisherman location)
					{
						fishingChair.worker.targetPosition = {col: rally_col, row: rally_row};
				...
				else if(workers) // we have some minions to work the field
				{
					for(var minion_ind:int = 0; minion_ind < workers.length; minion_ind++) // loop through the workers
					{
						...
						if(!curr_minion.isAssigned) // found an available worker
						{
							...
							fishingChair.worker.isAssigned = true;
							//update();
							break;
						}
